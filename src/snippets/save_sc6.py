...
# `Bills` is a mapping from username to list of lists of items
...
def _show_rows(rows: List[sqlite3.Row]) -> str:
  res = []
  for row in rows:
    ks = row.keys()
    res.append(str(ks))
    for k in ks:
      res.append(f'  {k}:{row[k]}')
    res.append('')
  return '\n'.join(res)

class Storage:
    """
    Store and retrieve orders in a sqlite DB
    by a customer within date.
    ...
    def _get(self, date: int) -> Bills:
        ...
        # logging.warning('date=%r, rows=%s', date, _show_rows(all_rows))
        for invoice_id, rows in itertools.groupby(cursor,
                operator.itemgetter('invoice_id')):
            for user, item_rows in itertools.groupby(rows,
    ...
    def bills_for_date(self, d: datetime.date
        ...
        """
        Retrieve all the bills for a particular date.
        """
        with contextlib.closing(self._open()):
            date = d.toordinal()
            bills = self._get(date)
        return bills

    def _new_invoice(self, date: int, user: str) -> int:
        cursor = self._db.cursor()
        cursor.execute('INSERT INTO Invoice(date,user) VALUES(:date,:user)',
                       {'date': date, 'user': user})
        return cursor.lastrowid

    def write_order(self, d: datetime.date, user: str,
        ...
        with contextlib.closing(self._open()):
            date = d.toordinal()
            invoice_id = self._new_invoice(date, user)
            cursor = self._db.cursor()

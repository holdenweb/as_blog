...
def total_sum1(items: List[PurchasedItem]) -> Decimal:
    ...
    270.66
    """
    return sum((it.unit_price * Decimal(it.units) for it in items), Decimal(0))
...
def total_sum2(items: List[PurchasedItem]) -> Decimal:
    ...
    return sum(
        (
            (it.unit_price * Decimal(it.units))
...
def total_sum3(items: List[PurchasedItem]) -> Decimal:
    ...
    297.726
    """
    return sum((post_tax_price(it, net_price(it)) for it in items), Decimal(0))

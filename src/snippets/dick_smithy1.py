...
# snippet dick-smithy1-2
from typing import List


def total_sum1(items: List[PurchasedItem]) -> Decimal:
    """Simple sum of total item prices. For example:
...
def total_sum2(items: List[PurchasedItem]) -> Decimal:
    ...
    """
    return sum(
        (it.unit_price * it.units)
        * Decimal(1 + Decimal(SALES_TAX_PERCENT.get(it.category, 6)) / 100)
        for it in items
    )


# snippet dick-smithy1-5
